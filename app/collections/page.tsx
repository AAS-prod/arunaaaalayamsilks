...
if (loading) {
return ( <div className="min-h-screen bg-[#160205] flex items-center justify-center text-white">
Loading Collections... </div>
);
}

return ( <main className="min-h-screen bg-[#160205] pt-36 pb-24 px-6 md:px-12 text-white"> <div className="max-w-7xl mx-auto">
    ...
    {sarees.length === 0 ? (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        {sarees.map((saree) => (
          <div
            key={saree.id}
            className="group rounded-2xl overflow-hidden border border-[#d4a24c]/10 bg-white/[0.01] hover:border-[#d4a24c]/30 transition duration-500"
          >
            <Link href={`/product/${saree.id}`}>
              <div className="aspect-[3/4] overflow-hidden bg-neutral-900 relative">
                ...
                <span className="absolute top-4 left-4 bg-black/60 backdrop-blur-md text-[#d4a24c] text-[10px] font-semibold tracking-widest uppercase px-3 py-1 rounded-full">
                  {saree.category}
                </span>
              </div>
            </Link>
            ...
            <div className="p-6">
              <Link href={`/product/${saree.id}`}>
                <h3 className="text-lg font-serif font-medium mb-2 hover:text-[#d4a24c]">
                  {saree.name}
                </h3>
              </Link>
